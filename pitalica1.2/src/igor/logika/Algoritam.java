...
public class Algoritam {
	...
	public final String lokacijaPitanja = "pitanja.txt";
	public final String lokacijaOcena = "ocene.txt";
	...
	public void dodajPitanjeUlistu(String tekst) {
		...
		// pitanje.setId(0);
		if (listaPitanja.size() == 0 || listaPitanja == null) {
			pitanje.setId(0);
		} else {
	...
	public void dodajOcenuUlistu(String tekst) {
		...
		if (listaOcena.size() == 0 || listaOcena == null) {
			ocena.setId(0);
		} else {
	...
	private void duplirajPitanjeUListi(int ocena, int id) {
		...
		if (pitanjaIzFajla.size() < 10) {
			...
			if (pitanjaIzFajla.size() < 35) {
				for (Pitanje pitanje : pitanjaIzFajla) {
					if (pitanje.getId() == id) {
						if (ocena == 2) {
							LinkedList<Pitanje> pitanjaZaUnos = new LinkedList<>();
							for (int i = 0; i < (pitanjaIzFajla.size() * 0.086); i++) {
								Pitanje p = new Pitanje();
								p.setId(pitanje.getId());
								p.setTekst(pitanje.getTekst());
								pitanjaZaUnos.add(p);
							}
							for (int i = 0; i < pitanjaZaUnos.size(); i++) {
								listaPitanja.add(pitanjaZaUnos.get(i));
							}
							...
							LinkedList<Pitanje> pitanjaZaUnos = new LinkedList<>();
							for (int i = 0; i < (pitanjaIzFajla.size() * 0.15); i++) {
								Pitanje p = new Pitanje();
								p.setId(pitanje.getId());
								p.setTekst(pitanje.getTekst());
								pitanjaZaUnos.add(p);
				...
				for (Pitanje pitanje : pitanjaIzFajla) {
					if (pitanje.getId() == id) {
						if (ocena == 2) {
							LinkedList<Pitanje> pitanjaZaUnos = new LinkedList<>();
							for (int i = 0; i < (pitanjaIzFajla.size() * 0.03); i++) {
								Pitanje p = new Pitanje();
								p.setId(pitanje.getId());
								p.setTekst(pitanje.getTekst());
								pitanjaZaUnos.add(p);
							...
							LinkedList<Pitanje> pitanjaZaUnos = new LinkedList<>();
							for (int i = 0; i < (pitanjaIzFajla.size() * 0.06); i++) {
								Pitanje p = new Pitanje();
								p.setId(pitanje.getId());
								p.setTekst(pitanje.getTekst());
								pitanjaZaUnos.add(p);
